...
const BlogDetail = () => {
  ...
  useSeo({
    title: currentBlog?.title || 'Blog',
    description: currentBlog?.excerpt,
    image: currentBlog?.coverImage ? getFileUrl(currentBlog.coverImage) : undefined,
    type: 'article'
  });
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        ...
        <div className="glass-card mb-8 p-8">
          {currentBlog.coverImage && (
            <img
              src={getFileUrl(currentBlog.coverImage)}
          ...
          <div className="flex items-center gap-3">
            {currentBlog.createdBy?.avatar ? (
              <img src={getFileUrl(currentBlog.createdBy.avatar)} alt={currentBlog.createdBy.name} className="w-10 h-10 rounded-full" />
            ) : (
