...
const Home = () => {
  ...
  useSeo({
    title: publicProfile?.name ? `${publicProfile.name} — Full-Stack Developer` : 'Full-Stack Developer Portfolio',
    description: publicProfile?.bio || 'Full-stack developer portfolio — projects, experience, skills, and certifications.'
  });
        ...
        <div className="w-full max-w-3xl text-center py-10">
          ...
          <Reveal delay={0.25} className="flex flex-col sm:flex-row gap-4 justify-center items-center">
            ...
            {publicProfile?.resume && (
              <Magnetic>
                <a
                  href={getFileUrl(publicProfile.resume)}
                  target="_blank"
