...
const Contact = () => {
  ...
  useSeo({
    title: 'Contact',
    description: 'Get in touch — send a message directly.'
  });

  // NOTE: this was previously `useState(() => {...}, [deps])`, which is a
  // hook mix-up — useState's initializer only runs once ever and ignores a
  // dependency array entirely, and its return value doesn't register a
  // cleanup function, so `clearSuccess()` never fired on unmount. useEffect
  // is the correct hook for a mount/unmount side effect.
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!validate()) return;
    // `error` from useSelector here is stale — it reflects state from the
    // render that created this closure, not the result of the dispatch
    // below. Checking the resolved action type is the reliable way to know
    // whether the request actually succeeded.
    const result = await dispatch(sendContact(formData));
